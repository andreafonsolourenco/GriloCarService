<%@ Page Language="C#" AutoEventWireup="true" CodeFile="lista_faturas_clientes.aspx.cs" Inherits="lista_faturas_clientes" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Grilo Car Service Software - Faturas dos Clientes">
    <meta name="author" content="André Lourenço">
    <title>Grilo Car Service Software - Faturas dos Clientes</title>
    <!-- Favicon -->
...
<body>
    ...
    <div class="main-content">
        ...
        <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <div class="container-fluid">
                <!-- Brand -->
                <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">Faturas de Clientes</a>
                <!-- Form -->
                    ...
                    <div class="card shadow">
                        <div class="card-header border-0">
                            <table style="width: 100%">
                                <tr>
                                    <td style="width: 65%; max-width: 50% !important;">
                                        <div style="float:left">
                                            <h3 class="mb-0">Faturas de Clientes</h3>
                                        </div>
                                    </td>
    ...
    <script>
        ...
        function getData() {
            loadingOn('A carregar as faturas dos clientes. Por favor aguarde...');
            ...
            if (ordCliente == 0 && ordNumero == 0 && ordData == 0 && ordDataVencimento == 0 && ordPaga == 0) {
                order = ' ORDER BY paga asc, data_vencimento asc, data_doc desc ';
            }
            else {
                order = ' ORDER BY ';
                ...
                if (ordCliente != 0) {
                    ...
                    order += ordData == -1 ? ' data_doc desc ' : ' data_doc asc ';
                }
            ...
            $.ajax({
                type: "POST",
                url: "lista_faturas_clientes.aspx/getGrelha",
        ...
        function editar(id, maintenance, sale) {
            if (maintenance) {
                loadUrl('config_ficha_fatura_fornecedor.aspx?id=' + id);
            }
        }

        function eliminar(id, maintenance, sale) {
            swal({
                title: 'Eliminar fatura de cliente',
                text: "A fatura de cliente será eliminada. Confirma?",
                type: 'question',
                ...
                if (isConfirm) {
                    delRow(id, maintenance, sale);
                }
            });
        }

        function delRow(id, maintenance, sale) {
            if (maintenance == '1') {
                $.ajax({
                    type: "POST",
                    url: "lista_faturas_clientes.aspx/delRow",
                    ...
                    success: function (res) {
                        ...
                        if (parseInt(res) <= 0) {
                            sweetAlertWarning("Eliminar fatura de cliente", resMsg);
                        }
                        else {
                            $('#txtPesquisa').val('');
                            ordCliente = 0;
        ...
        function visualizar(id, maintenance, sale) {
            var type = '';

            if (maintenance == '1') {
                type = 'FATURAS CLIENTES';
            } else if (sale == '1') {
                type = 'VENDAS';
            }
            ...
            $.ajax({
                ...
                data: '{"idUser":"' + localStorage.loga + '","id":"' + id + '","tipo":"' + type + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
                    var dados = res.d;
        ...
        function verDocs(id, maintenance, sale) {
            if (maintenance == '1') {
                $.ajax({
                    type: "POST",
                    url: "lista_faturas_clientes.aspx/getFiles",
        ...
        function simulateClickOnFileUploadButton(id, maintenance, sale) {
            $('#invoiceID').val(id);
        ...
        function gerarDadosPagamento() {
            ...
            for (i = 0; i < parseInt($('#countInvoices').html()); i++) {
                if ($('#invoiceSelectIcon' + i).hasClass('invoice_selected')) {
                    var maintenance = $('#maintenance' + i).html();
                    var sale = $('#sale' + i).html();
                    var id = $('#id' + i).html();

                    if (i > 0) {
                        if (maintenanceIds != '' && maintenance == '1') {
                            maintenanceIds += '<#SEP#>';
                        } else if (saleIds != '' && sale == '1') {
                            saleIds += '<#SEP#>';
                        }
                    }
            ...
            $.ajax({
                type: "POST",
                url: "lista_faturas_clientes.aspx/generatePaymentData",
                data: '{"idUser":"' + localStorage.loga + '","idsMaintenance":"' + maintenanceIds + '","idsSale":"' + saleIds + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
                    var dados = res.d.split('<#SEP#>');
        ...
        function pagarFaturas() {
            swal({
                title: "<strong>FATURAS DE CLIENTES</strong>",
                text: "Por favor, insira o método de pagamento para todas as faturas selecionadas!",
                input: 'text',
                showCancelButton: true,
                confirmButtonColor: '#007351',
                cancelButtonColor: '#d33',
        ...
        function payInvoice(xml) {
            $.ajax({
                type: "POST",
                url: "lista_faturas_clientes.aspx/payInvoice",
                data: '{"idUser":"' + localStorage.loga + '","xml":"' + xml + '"}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (res) {
        ...
        function generateXmlPayment(paymentMethod) {
            ...
            for (i = 0; i < parseInt($('#countInvoices').html()); i++) {
                if ($('#invoiceSelectIcon' + i).hasClass('invoice_selected')) {
                    xml += '<ID>' + $('#id' + i).html() + '</ID>';
                    xml += '<MAINTENANCE>' + $('#maintenance' + i).html() + '</MAINTENANCE>';
                    xml += '<SALE>' + $('#sale' + i).html() + '</SALE>';
                }
            }
