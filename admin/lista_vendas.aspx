<%@ Page Language="C#" AutoEventWireup="true" CodeFile="lista_vendas.aspx.cs" Inherits="lista_vendas" %>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
    <meta name="description" content="Grilo Car Service Software - Vendas">
    <meta name="author" content="André Lourenço">
    <title>Grilo Car Service Software - Vendas</title>
    <!-- Favicon -->
...
<body>
    ...
    <div class="main-content">
        ...
        <nav class="navbar navbar-top navbar-expand-md navbar-dark" id="navbar-main">
            <div class="container-fluid">
                <!-- Brand -->
                <a class="h4 mb-0 text-white text-uppercase d-none d-lg-inline-block">Vendas</a>
                <!-- Form -->
                                    ...
                                    <td style="width: 15%; max-width: 15% !important; text-align: right;">
                                        <div id="divCriaNovoPagamento">
                                            <a class="btn btn-sm btn-primary" onclick="novo();" style="color: #FFFFFF;">
                                                <span id="spanBtnAlteraStatus"><i class="fa fa-file-invoice" aria-hidden="true"></i>  Criar nova venda</span>
                                            </a>
                                        </div>
                                    </td>
                                </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td style="width: 40%; max-width: 40% !important; text-align: right;">
                                            <div>
                                                ...
                                                <asp:TextBox ID="saleID" runat="server" CssClass="variaveis" />
                                            </div>                                        
                                        </td>
                                    </tr>
                                </table>
    ...
    <script>
        ...
        function getData() {
            loadingOn('A carregar as vendas. Por favor aguarde...');
            ...
            if (ordCliente == 0 && ordData == 0 && ordDescricao == 0) {
                order = ' ORDER BY s.data_venda desc, s.cliente asc, s.descricao asc ';
            }
            else {
                order = ' ORDER BY ';

                if (ordCliente != 0) {
                    order += ordCliente == -1 ? ' s.cliente desc ' : ' s.cliente asc ';
                }
                else if (ordData != 0) {
                    order += ordData == -1 ? ' s.data_venda desc ' : ' s.data_venda asc ';
                }
                else if (ordDescricao != 0) {
                    order += ordDescricao == -1 ? ' s.descricao desc ' : ' s.descricao asc ';
                }
            }

            $.ajax({
                type: "POST",
                url: "lista_vendas.aspx/getGrelha",
        ...
        function editar(id) {
            loadUrl('config_ficha_venda.aspx?id=' + id);
        }

        function eliminar(id) {
            swal({
                title: 'Eliminar venda',
                text: "A venda será eliminada. Confirma?",
                type: 'question',
        ...
        function delRow(id) {
            $.ajax({
                ...
                success: function (res) {
                    ...
                    if (parseInt(res) <= 0) {
                        sweetAlertWarning("Eliminar venda", resMsg);
                    }
                    else {
                        $('#txtPesquisa').val('');
                        ordCliente = 0;
        ...
        function novo() {
            loadUrl('config_ficha_venda.aspx');
        }

        function visualizar(id) {
            $.ajax({
                type: "POST",
                url: "lista_vendas.aspx/generateViewInfo",
